(function ($) {
	...
	// Gán giá trị từ ViewBag
	const minVal = typeof minServer !== 'undefined' ? minServer : 0;
	const maxVal = typeof maxServer !== 'undefined' ? maxServer : 5000000;
	...
	// Gán giá trị ban đầu
	$("#amount").val(formatCurrency(minVal) + " - " + formatCurrency(maxVal));
	$("#minPrice").val(minVal);
	$("#maxPrice").val(maxVal);
	...
	$("#amount").on("change", function () {
		...
		let parts = raw.split("-");
		if (parts.length === 2) {
			let min = parseCurrency(parts[0]);
			let max = parseCurrency(parts[1]);
			if (!isNaN(min) && !isNaN(max) && min <= max) {
				$("#slider-range").slider("values", [min, max]);
				$("#minPrice").val(min);
				$("#maxPrice").val(max);
			} else {
				alert("Giá không hợp lệ. Hãy nhập theo định dạng: 1.000.000 ₫ - 5.000.000 ₫");
			}
		}
	});
	...
	$("form").on("submit", function () {
		...
		let parts = raw.split("-");
		if (parts.length === 2) {
			$("#minPrice").val(parseCurrency(parts[0]));
			$("#maxPrice").val(parseCurrency(parts[1]));
		}
	});
	/*----------------------------------------
		Bootstrap 5 Tooltip
...
function accordion(section, heading, list) {
	document.querySelectorAll(section).forEach(function (sec) {
		const head = sec.querySelector(heading);
		const lst = sec.querySelector(list);

		head.addEventListener("click", function () {
			head.classList.toggle("plus");

			if (lst.style.maxHeight) {
